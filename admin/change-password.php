...
session_start();
error_reporting(0);
require '../conFIle/conection.php'; // Update this file to use mysqli if not already
...
if (strlen($_SESSION['alogin']) == 0) {
	...
	if (isset($_POST['submit'])) {
		...
		$sql = "SELECT Password FROM admin_log WHERE UserName=? AND Password=?";
		...
		if ($stmt->num_rows > 0) {
			// Update password
			$sql = "UPDATE admin_log SET Password=? WHERE UserName=?";
			$stmt = $conn->prepare($sql);
	...
	<head>
		<title>LVA | Admin Change Password</title>
